...
3. **수정**: **opus 서브에이전트**를 Agent 도구로 호출하여 코드 수정을 위임한다.
   ```
   description: "feedback R{N}-{항목번호}: {항목 요약}"
   model: "opus"
   subagent_type: "general-purpose"
   prompt: |
     BetterAlarm/ 프로젝트의 Swift 코드를 수정하라.
     수정 대상: [관련 파일 경로 목록]
     피드백: [항목 내용]
     요구사항: Swift 6 + @MainActor + @Observable + PersonalColorDesignSystem 준수.
     파일을 읽고 Edit/Write 도구로 직접 수정하라. 커밋은 하지 마라.
   ```
   - 오케스트레이터(Sonnet)는 분류/우선순위/커밋만 담당. 코드 수정은 opus 서브에이전트가 담당.
   - 서브에이전트 결과(수정된 파일 목록)를 확인한 뒤 다음 단계로 진행.
4. **빌드 확인**: 오케스트레이터가 직접 `xcodebuild` 실행. 실패 시 opus 서브에이전트를 다시 호출하여 수정.
5. **harness/output 동기화**: 수정된 파일을 harness/output에도 반영
6. **커밋 — 항목 1개 = 커밋 1개** (오케스트레이터 Sonnet이 실행):
   ```bash
